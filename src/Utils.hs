...
instance (KnownSymbol s, Typeable c, Typeable a) => Todoable (Proxy '(s :: Symbol, c, a)) where
  todo Proxy = todo $ typeName @c <> "." <> symbolVal (Proxy @s) <>
    if typeName @a `Set.member` ignore then "" else " for " <> typeName @a
    where
